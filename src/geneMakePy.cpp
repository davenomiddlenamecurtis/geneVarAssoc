...
int refseqGeneInfo::writePyRegions(FILE *fo,writeBedFilePar &wbfp,char *arrayName)
{
	...
	if (gotAllExons==0)
		getAllExons();
	nUsedForKozak=0;
	//chr is written e.g. chr14 and we want e.g. 04
	if (strand=='+')
	{
	if (wbfp.write5UTR)
	{
		fprintf(fo,"        %s.add(self.siteQuerySelections.createChromInterval(\"%s\",%d%d));\n",arrayName,chr+3,transcript[0].txStart,transcript[0].cdsStart); // I think txStart and cdStart same for all transcripts
	}
	if (wbfp.write3UTR)
	{
		fprintf(fo,"        %s.add(self.siteQuerySelections.createChromInterval(\"%s\",%d%d));\n",arrayName,chr+3,transcript[0].cdsEnd,transcript[0].txEnd);
	}
	if (wbfp.writeKozaks)
	{
	...
	if (wbfp.writeSpliceSites)
	{
		...
		{
			s=exonEnds[e];
			fprintf(fo,"        %s.add(self.siteQuerySelections.createChromInterval(\"%s\",%d%d));\n",arrayName,chr+3,s-NSBDONOREXON,s+NSBDONORINTRON);
			s=exonStarts[e+1];
			fprintf(fo,"        %s.add(self.siteQuerySelections.createChromInterval(\"%s\",%d%d));\n",arrayName,chr+3,s-NSBACCEPTORINTRON,s+NSBACCEPTOREXON);
...
int main(int argc,char *argv[])
{
	...
	fp=fopen(argv[2],"r");
	fo=fopen("geneMakeBed.bed","w");
	feo=fopen("geneMakeBedExons.bed","w");
